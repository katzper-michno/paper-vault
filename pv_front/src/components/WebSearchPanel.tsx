...
export const WebSearchPanel: React.FC<WebSearchPanelProps> = ({ isOpen, savedIds, onSave }) => {
  ...
  useEffect(() => {
    setSearchResults((prev) =>
      prev
        ? prev.map((p) => (savedIds.has(p.id) ? { ...p, saved: true } : { ...p, saved: false }))
        : prev
    );
  }, [savedIds]);
  ...
  const handleSearch = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      const res = await axios.get<Paper[]>(
        `${SERVER_HOST}/search?q=${encodeURIComponent(searchQuery)}`
      );
      ...
      <div className="web-panel-inner">
        <div className="web-search-bar">
          <form onSubmit={handleSearch} className="web-input-wrap">
            ...
            />
            <button type="submit" disabled={searching} className="go-btn">
